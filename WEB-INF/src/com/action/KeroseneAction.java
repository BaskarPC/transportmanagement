...
public class KeroseneAction extends BaseDispatchAction{
	private static final Logger logger=(Logger)Logger.getLogger(KeroseneAction.class);
	job jb=new job();
	int jobid;
	dbcom db=new dbcom();
	...
	/**
	 * to list the values while loading
	 */
	public ActionForward defaultMethod(ActionMapping mapping,
			...
			KeroseneForm kf=(KeroseneForm)form;
			arrayList list=new arrayList();
			list.dropdownList(form);
			
			reasonList=keroseneservice.fetchKeroseneReason();
			logger.debug("action in"+reasonList.size());
			kf.setReasonList(reasonList);
			reasonList=keroseneservice.fetchRates("P6");
			vo=(KeroseneIssueVO)reasonList.get(0);
			kf.setOldAvailable(vo.getOldAvailable());
			kf.setNewAvailable(vo.getNewAvailable());
			kf.setOldRate(vo.getOldRate());
			kf.setNewRate(vo.getNewRate());
			int total=Integer.parseInt(vo.getOldAvailable())+Integer.parseInt(vo.getNewAvailable());
			kf.setTotalAvailableQuantity(Integer.toString(total));
	...
	public ActionForward listVehicleMethod(ActionMapping mapping,
			...
			ArrayList list=new ArrayList();
			KeroseneForm sf=(KeroseneForm)form;
	...
	public void insertvalues(int jobid,ActionForm form) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException
	{
		...
		try
	    {
			KeroseneIssueService ss=new KeroseneIssueService();
	...
	public ActionForward qtyUpdateMethod(ActionMapping mapping,
		...
		{	
			if(Integer.parseInt(sf.getOldAvailable())!=0)
			{
				...
				ss.updateOldQuantityValues(7,oldtemp);
			}
			...
			if(Integer.parseInt(sf.getNewAvailable())!=0)
			{
				if(temp!=0)
				{
					...
					ss.updateNewQuantityValues(7,newtemp);
				}
			}
		}
	...
	public ActionForward inboxMethod(ActionMapping mapping,
	    ...
	    sf.setKeroseneReason(kvo.getKeroseneReason());
